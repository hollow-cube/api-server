...
func getRRW(writer http.ResponseWriter, ctx context.Context, propagator propagation.TextMapPropagator) *recordingResponseWriter {
		...
		WriteHeader: func(next httpsnoop.WriteHeaderFunc) httpsnoop.WriteHeaderFunc {
			return func(statusCode int) {
				if !rrw.written {
					...
					// Inject the propagation header for another svc to handle
					propagator.Inject(ctx, propagation.HeaderCarrier(writer.Header()))
					// Add the trace id to the response header for debugging
					span := oteltrace.SpanFromContext(ctx)
					if span.SpanContext().HasTraceID() {
						writer.Header().Set("x-trace-id", span.SpanContext().TraceID().String())
					}
				}
				next(statusCode)
			}
...
func (tw traceware) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	rrw := getRRW(w, ctx, tw.propagators)
	defer putRRW(rrw)
