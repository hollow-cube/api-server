...
func (s *server) applyChangeList(ctx context.Context, rawEvent *tebex.Event, txId string, changes []*model.TebexChange) (map[string]int, error) {
	return playerdb.Tx(ctx, s.store, func(ctx context.Context, tx *playerdb.Store) (newBalances map[string]int, err error) { // 2pc: Begin transaction
...
func applyCubitsChanges(ctx context.Context, store *playerdb.Store, changes []*model.TebexChange, txMeta map[string]interface{}) (newBalances map[string]int, err error) {
	...
	for _, change := range changes {
		...
		newBalances[change.Target], err = store.AddCurrency(
			ctx, change.Target, playerdb.Cubits, change.Amount,
			playerdb.BalanceChangeReasonTebexOneoff, txMeta,
		)
		if err != nil {
			return
		}
...
func logTebexEvent(ctx context.Context, store *playerdb.Store, event *tebex.Event) error {
