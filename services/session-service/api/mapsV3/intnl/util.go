...
func (s *server) getUnlockedSlots(ctx context.Context, pd *mapdb.MapPlayerData) (int, error) {
...
func (s *server) getTotalSlotsFromPerm(ctx context.Context, mpd *mapdb.MapPlayerData) (int, error) {
	pd, err := s.playerStore.GetPlayerData(ctx, mpd.ID)
	if err != nil {
		return 0, err
	}

	return pd.TotalMapSlots(), nil
}
