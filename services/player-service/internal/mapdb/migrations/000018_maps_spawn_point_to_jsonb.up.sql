...
alter table maps
  alter column opt_spawn_point type jsonb
    using opt_spawn_point::jsonb;

alter table maps
  alter column ext drop default;
alter table maps
  alter column ext type jsonb
    using convert_from(ext, 'UTF8')::jsonb;;
alter table maps
  alter column ext set default '{}'::jsonb;
...
select m.id,
       ...
       coalesce(stats.win_count, 0) as clear_rate,
       case
