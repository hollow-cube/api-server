...
func (q *Queries) DeleteNotification(ctx context.Context, playerID string, iD string) ([]string, error) {
	rows, err := q.db.Query(ctx, deleteNotification, playerID, iD)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const deleteNotifications = `-- name: DeleteNotifications :many
update player_notifications
set deleted_at = now()
where player_id = $1
  and type = $2
...
func (q *Queries) DeleteNotifications(ctx context.Context, playerID string, type_ string, key string) ([]string, error) {
	rows, err := q.db.Query(ctx, deleteNotifications, playerID, type_, key)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
